...
# Optional: pin to a release tag, e.g. v1.0.20260904.123-a1b2c3d.
...
validate_app_version() {
  if [ -n "${APP_VERSION}" ] && ! [[ "${APP_VERSION}" =~ ^v1\.0\.[0-9]{8}\.[0-9]+-[0-9a-f]{7}$ ]]; then
    echo -e "${RED}APP_VERSION must be v1.0.yyyyMMdd.<commit-count>-<7-character-git-hash>.${PLAIN}" >&2
    exit 1
  fi
}
...
install_binary() {
  ...
  if [ -f "./bin/${APP_NAME}_linux_${goarch}" ]; then
    ...
    source_version="v1.0.$(date -u +%Y%m%d).$(git rev-list --count HEAD 2>/dev/null || printf '0')-$(git rev-parse --short=7 HEAD 2>/dev/null || printf 'local')"
